...
class gcc_ca_mf_block_onnx(BaseModule):
    def __init__(self,
        ...
        self.conv_1_H = nn.Conv2d(dim, dim, (meta_kernel_size//2, 1),
         groups=dim,padding=0)
        self.conv_1_W = nn.Conv2d(dim, dim, (1, meta_kernel_size//2),
         groups=dim,padding=0)
        self.conv_2_H = nn.Conv2d(dim, dim, (meta_kernel_size//2, 1),
        groups=dim,padding=0)
        self.conv_2_W = nn.Conv2d(dim, dim, (1, meta_kernel_size//2), 
        groups=dim,padding=0)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # stage 2
        x_1_2 = self.conv_2_W(torch.cat((x_1_1, x_1_1[:, :, :, :-1]), dim=3))
        x_2_2 = self.conv_2_H(torch.cat((x_2_1, x_2_1[:, :, :-1, :]), dim=2))
